...
function storyboard(value,duration){
 ...
 const scenes=value.scenes.map((s,i)=>{if(typeof s.description!=='string'||!s.description.trim()||s.description.length>3000||!Number.isFinite(s.duration)||s.duration<=0||s.duration>15)throw new Error('Проверьте описание и длительность сцен');return {id:String(i+1),description:s.description.trim(),duration:s.duration,heading:String(s.heading||`Сцена ${i+1}`).slice(0,160),...Object.fromEntries(['voiceover','sound','transition','motion','composition','characters','camera'].filter(key=>typeof s[key]==='string').map(key=>[key,s[key].slice(0,4000)]))};});
 ...
 return {title:String(value.title||'Видео').slice(0,200),caption:String(value.caption||'').slice(0,5000),style:String(value.style||'').slice(0,5000),music:String(value.music||'').slice(0,2000),scenes};
}
...
export function createCreativeHandler({adapter=createGatewayAdapter(),resolveAdapter,resolveCapabilities,ttlMs=3600000,maxJobs=40}={}){
 const jobs=new Map();
 ...
 async function render(job,onlyScene){
  ...
  advance(job,'visuals');
  for(const scene of target){job.abort.signal.throwIfAborted();const index=job.scenes.indexOf(scene);const result=await adapter.keyframe({brief:job.input,style:job.style,scene,previous:job.scenes[index-1]?.image,first:job.scenes[0]?.image},job.abort.signal);scene.image=media(result.image,'image');}
  advance(job,'animation');
  for(const scene of target){job.abort.signal.throwIfAborted();const index=job.scenes.indexOf(scene);const result=await adapter.animate({brief:job.input,style:job.style,scene,previous:job.scenes[index-1]?.clip},job.abort.signal);scene.clip=media(result.video,'video');}
  advance(job,'montage');
  const result=await adapter.compose({brief:job.input,title:job.title,caption:job.caption,music:job.music,style:job.style,scenes:job.scenes},job.abort.signal);
 ...
 return async function creative(req,res,next){
  ...
  try{
   ...
   if(path==='/api/creative/capabilities'&&req.method==='GET'){const capabilities=adapter?undefined:resolveCapabilities?.(req);return reply(res,200,{configured:capabilities?!!(capabilities.image||capabilities.video):!!adapter,kinds:capabilities?['image','video'].filter(kind=>capabilities[kind]):['image','video'],referenceLimit:10,durations:[15,30,45,60]});}
   if(path==='/api/creative/jobs'&&req.method==='POST'){
    const input=validate(await body(req));const selectedAdapter=resolveAdapter?.(req,input)||adapter;const capabilities=adapter?undefined:resolveCapabilities?.(req);if(!selectedAdapter||capabilities&&!capabilities[input.kind])throw new RequestError('Сервис генерации ещё не подключён. Идея и референсы сохранены в этом окне.',503);
    if(jobs.size>=maxJobs)throw new RequestError('Слишком много задач. Повторите позже.',429);
    const job={id:randomUUID(),token:randomBytes(32).toString('hex'),adapter:selectedAdapter,input,status:'running',stage:input.kind==='image'?'visuals':'script',scenes:[],abort:new AbortController(),updatedAt:Date.now()};jobs.set(job.id,job);
    run(job,async()=>{const adapter=job.adapter;if(input.kind==='image'){const result=await adapter.image(input,job.abort.signal);job.result=media(result.image,'image');advance(job,'ready');job.status='complete';}else{const script=storyboard(await adapter.storyboard(input,job.abort.signal),input.duration);Object.assign(job,script);if(input.mode==='review'){job.status='review';}else await render(job);}});
   ...
   if(req.method==='POST'&&match[2]==='continue'){
    ...
    const input=await body(req);let script;try{script=storyboard({...input,title:job.title,caption:job.caption},job.input.duration);}catch(error){throw new RequestError(error.message);}if(job.status!=='review')throw new RequestError('Сценарий уже передан в генерацию',409);job.scenes=script.scenes.map((scene,index)=>job.scenes[index]?.description===scene.description?{...job.scenes[index],...scene}:scene);job.status='running';run(job,()=>render(job));return reply(res,202,publicJob(job));
   }
   if(req.method==='POST'&&match[2]==='scene'){
    ...
    const input=await body(req);if(!['complete','error'].includes(job.status))throw new RequestError('Дождитесь готового видео',409);const scene=job.scenes.find(s=>s.id===input.id);if(!scene||typeof input.description!=='string'||!input.description.trim()||input.description.length>3000)throw new RequestError('Добавьте описание сцены');
    scene.description=input.description.trim();for(const key of ['voiceover','sound','transition','motion','composition','characters','camera'])delete scene[key];job.status='running';run(job,()=>render(job,scene.id));return reply(res,202,publicJob(job));
   }
