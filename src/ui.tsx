...
export function PlatformIcon({platform, size = 24}: {platform: Platform; size?: number}) {
  return <span className={`platform-icon platform-${platform}`} style={{width: size, height: size, fontSize: size * .37}} title={platformMeta[platform].name} aria-label={platformMeta[platform].name}>{platform === 'telegram' ? <Send size={size * .53} fill="currentColor" strokeWidth={1.4}/> : platform === 'instagram' ? <Instagram size={size * .58} strokeWidth={1.6}/> : platform === 'youtube' ? <Youtube size={size * .62} strokeWidth={1.8}/> : <b>{platform === 'vk' ? 'vk' : platform === 'threads' ? '@' : 'дз'}</b>}</span>;
}
...
export function Avatar({name, size = 30}: {name: string; size?: number}) { return <span className={`avatar avatar-${name.length % 4}`} title={name} style={{width: size, height: size, fontSize: size * .34}}>{name.slice(0, 1)}</span>; }
