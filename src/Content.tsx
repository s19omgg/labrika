...
export default function Content() {
  ...
  return <div className="content-page page-enter">
    <div className="page-heading"><h1 className="page-title">Публикации <span className="content-total">{posts.length}</span></h1><button className="btn btn-primary" onClick={() => openEditor()}><Plus size={17}/> Создать пост</button></div>
    ...
    {board ? <div className="content-board">{statuses.map(status => <section className={`content-board-column ${dragOver === status ? 'is-drag-over' : ''}`} key={status} onDragOver={event => {if (status !== 'published') {event.preventDefault();setDragOver(status);}}} onDragLeave={event => {if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragOver(null);}} onDrop={event => {event.preventDefault();changeStatus(event.dataTransfer.getData('text/plain'), status);setDragOver(null);}}>
      ...
      <div className="content-board-stack">{filteredPosts.filter(post => post.status === status).map(post => <article className="content-board-card" key={post.id} draggable={post.status !== 'published'} onDragStart={event => {event.dataTransfer.setData('text/plain', post.id);event.dataTransfer.effectAllowed = 'move';}} onDragEnd={() => setDragOver(null)}>
        ...
        <div className="content-board-card-bottom"><div className="content-platforms">{post.platforms.map(value => <PlatformIcon key={value} platform={value} size={23}/>)}</div><span>{dateLabel(post.date)}</span>{post.status !== 'published' && <GripVertical size={14}/>}</div>
        <div className="content-board-move"><span>Этап</span><Select className="content-board-stage" aria-label={`Изменить этап: ${post.title}`} value={post.status} disabled={post.status === 'published'} onChange={value => changeStatus(post.id, value as PostStatus)} options={statuses.map(value => ({value, label: statusMeta[value].name, disabled: value === 'published'}))}/></div>
      </article>)}{filteredPosts.every(post => post.status !== status) && <div className="content-board-empty">{status === 'published' ? 'Здесь появятся опубликованные посты' : 'Перетащите публикацию сюда'}</div>}</div>
