...
export function createVisual(input: VisualInput): GeneratedVisual {
  ...
  const brandLockup = `<rect x="64" y="60" width="46" height="46" rx="14" fill="#d5ff3f"/><path d="M77 73 87 83 97 73M87 83v13" fill="none" stroke="#18191c" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>${svgText(brand,126,92,27,550,1,'#18191c',600)}`;
  ...
  if (input.kind === 'minimal') {
    artwork = `<rect width="1080" height="1080" fill="#f8f9fb"/><circle cx="903" cy="303" r="315" fill="#d5ff3f"/><circle cx="903" cy="303" r="210" fill="none" stroke="#18191c" stroke-opacity=".11" stroke-width="2"/><circle cx="903" cy="303" r="115" fill="none" stroke="#18191c" stroke-opacity=".11" stroke-width="2"/>${brandLockup}${svgText(title,64,394,87,920,4)}${svgText(input.subtitle,69,861,26,790,3,'#6e727c',400)}${footer}`;
  } else if (input.kind === 'statistic') {
    artwork = `<rect width="1080" height="1080" fill="#fafafa"/>${brandLockup}${svgText(title,64,220,48,880,3)}<rect x="64" y="410" width="952" height="383" rx="38" fill="#18191c"/>${svgText(input.metric || '—',109,648,164,850,1,'#d5ff3f',500)}<path d="M860 531h74v74m0-74-74 74" fill="none" stroke="#d5ff3f" stroke-width="7"/>${svgText(input.subtitle,69,869,27,860,3,'#777b83',400)}${footer}`;
  } else if (input.kind === 'infographic') {
    artwork = `<rect width="1080" height="1350" fill="#f7f8fa"/>${brandLockup}${svgText(title,64,220,62,900,3)}${input.points.slice(0,3).map((point,index)=>`<rect x="64" y="${460+index*224}" width="952" height="198" rx="30" fill="${index===0?'#d5ff3f':'#fff'}"/><circle cx="128" cy="${520+index*224}" r="26" fill="#18191c"/>${svgText(`0${index+1}`,111,528+index*224,21,70,1,'#fff',500)}${svgText(point,187,518+index*224,35,765,3,'#25282e',500,1.3)}`).join('')}${svgText(input.subtitle,69,1202,23,840,2,'#888c94',400)}${footer}`;
  } else if (input.kind === 'cover') {
    artwork = `<rect width="1280" height="720" fill="#f7f8fa"/><rect x="917" y="0" width="363" height="720" fill="#367cfa"/><circle cx="1130" cy="370" r="255" fill="none" stroke="#fff" stroke-opacity=".2" stroke-width="2"/><circle cx="1130" cy="370" r="163" fill="none" stroke="#fff" stroke-opacity=".24" stroke-width="2"/><path d="M1046 332h151v151m0-151-151 151" fill="none" stroke="#d5ff3f" stroke-width="15"/>${brandLockup}${svgText(title,64,280,73,825,4)}${svgText(input.subtitle,68,593,23,780,2,'#777c86',400)}${svgText(brand.toLocaleUpperCase('ru-RU'),64,666,16,650,1,'#989da5',400)}`;
  } else {
    artwork = `<rect width="1080" height="1920" fill="#f8f9fb"/>${brandLockup}<circle cx="997" cy="583" r="382" fill="#d5ff3f"/><circle cx="997" cy="583" r="275" fill="none" stroke="#18191c" stroke-opacity=".09" stroke-width="2"/>${svgText(title,64,530,103,920,6)}<rect x="64" y="1285" width="952" height="350" rx="36" fill="#18191c"/>${svgText(input.subtitle || 'Одна идея. Следующий шаг. Ваш результат.',103,1370,42,865,4,'#fff',400,1.35)}<circle cx="922" cy="1573" r="28" fill="#d5ff3f"/>${footer}`;
  }
...
function drawVideoFrame(ctx: CanvasRenderingContext2D, script: VideoScript, brand: string, elapsed: number, total: number) {
  ...
  ctx.save();ctx.translate(440,283);ctx.rotate(elapsed*.07);ctx.fillStyle=isDark?'#367cfa':'#d5ff3f';ctx.beginPath();ctx.arc(0,0,187+Math.sin(elapsed)*8,0,Math.PI*2);ctx.fill();ctx.strokeStyle=isDark?'#ffffff22':'#18191c16';ctx.lineWidth=1;ctx.beginPath();ctx.arc(0,0,125,0,Math.PI*2);ctx.stroke();ctx.restore();
  ctx.fillStyle='#d5ff3f';ctx.beginPath();ctx.roundRect(34,37,32,32,9);ctx.fill();ctx.strokeStyle='#18191c';ctx.lineWidth=3;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(43,47);ctx.lineTo(50,54);ctx.lineTo(57,47);ctx.moveTo(50,54);ctx.lineTo(50,62);ctx.stroke();
  ...
  script.scenes.forEach((_,i)=>{ctx.fillStyle=i===index?'#d5ff3f':isDark?'#ffffff28':'#18191c20';ctx.beginPath();ctx.arc(width-40-(script.scenes.length-1-i)*16,876,4,0,Math.PI*2);ctx.fill();});
