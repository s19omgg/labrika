...
function Overview({ onCompany, onCreate }: { onCompany: () => void; onCreate: () => void }) {
  return <div className="ai-bento">
    <section className="ai-bento-card ai-intro">
      <span className="ai-card-label"><BrainCircuit size={17} /> Интеллект внутри LABRIKA</span>
      ...
      <button className="ai-intro-link" onClick={onCompany}>Что LABRIKA знает о вас <ArrowUpRight size={18} /></button>
    </section>
...
export default function LabrikaAI() {
  ...
  const products = verified.filter(item => item.category === 'product' || item.category === 'service');
  ...
  return <div className="ai-page ai-page-v2">
    <div className="page-heading"><h1 className="page-title">LABRIKA AI</h1>{section === 'company' && <button className="ai-header-action" onClick={() => navigate('brain')}>База знаний <ArrowUpRight size={16} /></button>}</div>
    {section !== 'company' ? <Overview onCompany={() => setSection('company')} onCreate={() => navigate('controller')} /> : <div className="ai-company-grid">
      ...
      <section className="ai-company-card ai-company-products">
        ...
        {products.length ? <div className="ai-record-list">{products.slice(0, 5).map(item => <article key={item.id}><span className="ai-record-icon"><Layers3 size={19} /></span><div><span className="ai-record-category">{knowledgeLabels[item.category]}</span><h4>{item.title}</h4><p>{item.content}</p></div></article>)}</div> : <div className="ai-data-empty"><Layers3 size={26} /><p>Какие задачи решает ваш продукт?</p><button onClick={() => navigate('brain')}>Добавить сведения <Plus size={15} /></button></div>}
      ...
      <section className="ai-company-card ai-company-history">
        ...
        {recent.length ? <div className="ai-learning-list">{recent.map(event => <div key={event.id}><span>{event.type === 'edit' ? <PencilLine size={15} /> : <CheckCheck size={15} />}</span><div><strong>{event.type === 'edit' ? 'Сохранены правки текста' : 'Выбран вариант публикации'}</strong><p>{event.type === 'edit' && event.beforeLength !== undefined && event.afterLength !== undefined ? `${formatNumber(event.beforeLength)} → ${formatNumber(event.afterLength)} символов` : [event.tone, event.goal].filter(Boolean).join(' · ') || 'Выбор добавлен в предпочтения'}</p></div><time dateTime={event.at}>{dateLabel(event.at)}</time></div>)}</div> : <div className="ai-data-empty"><Sparkles size={25} /><p>История пока чистая</p><span>Здесь будут ваши выбранные варианты и сохранённые изменения</span></div>}
      </section>
    </div>}
  </div>;
}
