...
export type Platform = 'telegram' | 'instagram' | 'vk' | 'youtube' | 'threads' | 'dzen';
...
export const seedPosts: Post[] = [
  ...publishedTitles.map((title, i) => makePost(title, {
    ...
    body: `${title}\n\nХороший контент начинается с понимания людей. Мы собираем идеи, проверяем гипотезы и делимся тем, что помогает двигаться вперёд.\n\nВ LABRIKA каждый материал — часть общей истории. Рассказываем о подходе, который помогает команде создавать полезное и видеть результат.\n\nСохраните, чтобы вернуться к этим идеям.`, cover: ['lavender','peach','mint','blue'][i%4],
  })),
  ...['Как превратить подписчиков в клиентов', 'За кулисами LABRIKA', 'Три привычки сильной контент-команды', 'Новая глава: обновление платформы', 'История одного запуска', 'От данных к хорошим решениям', 'Ответы на ваши вопросы', 'Контент, к которому возвращаются'].map((title, i) => makePost(title, {
