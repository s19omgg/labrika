...
import { LayoutDashboard, SlidersHorizontal, CalendarDays, ChartNoAxesCombined, Settings2, ChevronDown, Search, ArrowUpRight, Command, Menu, Check, CircleHelp, PanelLeftClose, PanelLeftOpen, UserRound, LogOut, ArrowRight, BrainCircuit, Workflow, Grid2X2, Globe2, ScanEye, Repeat2 } from 'lucide-react';
...
const navigation = [
  ...
  {id:'controller' as Page,name:'Контроллер',icon:SlidersHorizontal},
...
const sections:Record<Page,{id:string;name:string}[]>={
  ...
  settings:[{id:'company',name:'Компания'},{id:'social',name:'Социальные сети'},{id:'workspace',name:'Пространство'}],
  profile:[{id:'personal',name:'Профиль'}],
  brain:[{id:'overview',name:'Обзор'},{id:'knowledge',name:'Знания'},{id:'rules',name:'Правила'},{id:'sources',name:'Источники'}],
  ...
  campaigns:[{id:'all',name:'Кампании'}],repurpose:[{id:'all',name:'Repurpose'}],ai:[{id:'overview',name:'Обзор'},{id:'company',name:'О компании'}],billing:[{id:'plans',name:'Тарифы'}],
};
function Brand({onClick}: {onClick:()=>void}) {return <button className="brand-lockup" onClick={onClick} aria-label={`${productName} — на главную`}><span className="brand-mark"><svg viewBox="0 0 32 32" fill="none"><path d="M16 2 28 9v14L16 30 4 23V9L16 2Z" fill="currentColor"/><path d={isLabrika?"M12 9v14h9":"m10 11 6 6 6-6M16 17v7"} stroke="#caff52" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span>{productName.toLowerCase()}<span className="brand-dot">.</span></span></button>;}
export default function App() {
  useAutopilotRunner();
  const {page,navigate,posts,openEditor,editorOpen,section,setSection,profile} = useWorkspace();
  ...
  return <div className={`app-layout design-v2 workspace-v3 workspace-v5 workspace-v6 ${expanded?'sidebar-expanded':''}`}>
