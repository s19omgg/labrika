...
export default function Dashboard() {
  ...
  const [range,setRange] = useState<DateRange>(()=>({start:monthStart,end:today}));
  const firstDate = range.start, lastDate = range.end;
  const choosePeriod=(next:'month'|'week')=>{setPeriod(next);setRange({start:next==='month'?monthStart:weekStart,end:today});};
  const published = posts.filter(post => post.status === 'published' && post.date >= firstDate && post.date <= lastDate);
  ...
  return <div className="dash-home" aria-label={`Обзор ${productName}`}>
    <div className="page-heading dash-page-heading">
      ...
      <div className="dash-heading-actions">
        <DateRangePicker value={range} max={today} onChange={next=>{setRange(next);setPeriod('custom');}}/>
    ...
    <div className="dash-layout">
      <div className="dash-main-grid">
        ...
        <section className="dash-card dash-growth">
          <div className="dash-card-heading"><h2><span className="dash-chart-mark"><ChartNoAxesColumnIncreasing size={16} strokeWidth={1.5} /></span>Динамика</h2><div className="dash-period-tabs" role="group" aria-label="Период графика"><button className={period === 'week' ? 'active' : ''} aria-pressed={period === 'week'} onClick={() => choosePeriod('week')}>Неделя</button><button className={period === 'month' ? 'active' : ''} aria-pressed={period === 'month'} onClick={() => choosePeriod('month')}>Месяц</button></div></div>
          ...
          {published.length ? <GrowthBars posts={published} period={period} metric={metric} firstDate={firstDate} today={lastDate} /> : <div className="dash-empty" style={{flex: 1, flexDirection: 'column'}}><ChartNoAxesColumnIncreasing size={30} strokeWidth={1.4}/><span>Данных за период пока нет</span></div>}
        ...
        <section className="dash-card dash-upcoming">
          ...
          {!scheduled.length && <div className="dash-empty dash-upcoming-empty"><div className="dash-upcoming-empty-content"><span className="dash-upcoming-empty-message"><CalendarDays size={24} /><span>Запланируйте первую публикацию</span></span><button onClick={() => openEditor()}>Создать пост<Plus size={14} /></button></div></div>}
        </section>
      </div>
...
function GrowthBars({ posts, period, metric, firstDate, today }: { posts: Post[]; period: Period; metric: Metric; firstDate: string; today: string }) {
  ...
  const values = Array.from({ length: pointCount }, (_, index) => {
    const day = new Date(start); day.setDate(day.getDate() + (pointCount===1?0:Math.round(index * dayCount / (pointCount-1))));
