...
import { ArrowUpRight, Send, Bold, CalendarDays, Check, Clock3, Copy, Eye, Film, Image, Italic, List, MousePointer2, Plus, Sparkles, Trash2, UserRoundPlus, X } from 'lucide-react';
...
const limits: Partial<Record<Platform, number>> = {telegram: 4096, instagram: 2200, vk: 16384, youtube: 5000, threads: 500, max:4000, tiktok:2200};
...
function PublishedPost({post}: {post: Post}) {
  const {brand, openEditor, notify} = useWorkspace();const {can}=useTeamAccess();
  ...
  return <div className="published-detail">
    ...
    {post.deliveries?.[platform]?.url&&<div className="published-remote-link"><a href={post.deliveries[platform]!.url} target="_blank" rel="noreferrer">Открыть публикацию<ArrowUpRight size={15}/></a></div>}
    <div className="published-footer"><button className="btn btn-ghost" onClick={() => copyText(body, notify)}><Copy size={15}/> Копировать текст</button><button disabled={!can('createPosts')} className="btn btn-primary" onClick={() => openEditor(makePost(post.title, {body: post.body, platformBodies: post.platformBodies, platforms: [...post.platforms], format: post.format, topic: post.topic, cover: post.cover, goal: post.goal, media: post.media, videoScript: post.videoScript}))}><Plus size={15}/> Создать на основе</button></div>
  </div>;
}
...
function EditorForm({initial}: {initial: Post | null}) {
  ...
  const {can}=useTeamAccess();const canEdit=can('createPosts'),canApprove=can('approvePosts'),canSchedule=can('schedulePosts');
  ...
  const source=posts.find(item=>item.id===post.id);const existing=!!source;
  const maySetStatus=(status:PostStatus)=>status==='draft'?canEdit:status==='approved'?canApprove:status==='scheduled'?canSchedule:false;
  const canSave=existing?(canEdit||canApprove||canSchedule):canEdit;
  const patch = (value: Partial<Post>) => {if(Object.keys(value).some(key=>key==='status'?!maySetStatus(value.status!):['date','time','planned'].includes(key)?!canSchedule:!canEdit))return;setPost(current => ({...current, ...value, ...(value.body !== undefined ? {platformBodies: undefined} : {})}));setError('');};
  ...
  const save = () => {
    if(!canSave||post.status!==source?.status&&!maySetStatus(post.status)){setError('Недостаточно прав для этого действия');return;}
    ...
    if (existing && source) {const changes=Object.fromEntries(Object.entries(finalPost).filter(([key,value])=>JSON.stringify(value)!==JSON.stringify(source[key as keyof Post]))) as Partial<Post>;if(!canEdit)for(const key of Object.keys(changes))if(!['status','date','time','planned'].includes(key))delete changes[key as keyof Post];if(!Object.keys(changes).length){closeEditor();return;}updatePost(post.id, changes);} else addPost(finalPost);
    closeEditor();
  ...
  return <div className="editor-shell editor-v3">
    <div className="editor-columns"><div className="editor-main">
      ...
      <input readOnly={!canEdit} id="post-title" className="editor-title-input" value={post.title} onChange={event => patch({title: event.target.value})} placeholder="О чём расскажем?" autoFocus/>
      <div className="editor-classification"><div><span>Формат</span><Select disabled={!canEdit} aria-label="Формат публикации" value={post.format} onChange={value => patch({format: value})} options={formats.map(value => ({value, label: value}))}/></div><div><span>Тема</span><Select disabled={!canEdit} aria-label="Тема публикации" value={post.topic} onChange={value => patch({topic: value})} options={topics.map(value => ({value, label: value}))}/></div></div>
      ...
      <div className="editor-writing"><div className="editor-format-toolbar"><button disabled={!canEdit} title="Жирный текст" aria-label="Жирный текст" onClick={() => wrapSelection('**', '**')}><Bold size={16}/></button><button disabled={!canEdit} title="Курсив" aria-label="Курсив" onClick={() => wrapSelection('_', '_')}><Italic size={16}/></button><span/><button disabled={!canEdit} title="Добавить пункт списка" aria-label="Добавить пункт списка" onClick={() => wrapSelection('\n• ', '')}><List size={17}/></button></div><textarea readOnly={!canEdit} id="post-body" ref={bodyRef} value={post.body} onChange={event => patch({body: event.target.value})} placeholder="Начните с главного. Что вы хотите рассказать?"/><div className="editor-writing-bottom"><span>{post.body.trim() ? post.body.trim().split(/\s+/).length : 0} слов</span><span className={previewLimit && previewBody.length > previewLimit ? 'editor-over-limit' : ''}>{formatNumber(previewBody.length)}{previewLimit ? ` / ${formatNumber(previewLimit)}` : ' символов'}</span></div></div>
      {post.media && <div className="editor-media-attachment">{post.media.kind === 'image' ? <img src={post.media.url} alt=""/> : <span className="editor-media-video"><Film size={19}/></span>}<span><strong>{post.media.kind === 'image' ? 'Изображение' : 'Видео'}</strong><small>{post.media.format}</small></span>{post.media.kind === 'image' ? <Image size={16}/> : <Film size={16}/>}<button disabled={!canEdit} className="icon-button" aria-label="Удалить медиа из публикации" onClick={() => patch({media: undefined})}><X size={16}/></button></div>}
      ...
      <div className="editor-label-row editor-platform-label"><span className="editor-field-label">Площадки</span></div><div className="editor-platform-selection">{platforms.map(platform => <button disabled={!canEdit} className={post.platforms.includes(platform) ? 'is-selected' : ''} key={platform} aria-pressed={post.platforms.includes(platform)} onClick={() => togglePlatform(platform)}><PlatformIcon platform={platform} size={23}/><span>{platformMeta[platform].name}</span>{post.platforms.includes(platform) && <Check size={13}/>}</button>)}</div>
      <ContentHealth readOnly={!canEdit} title={post.title} body={post.body} media={post.media} postId={post.id} goal={post.goal} onChange={body=>patch({body})}/><div className="editor-brand-note"><span className="editor-brand-spark"><Sparkles size={14}/></span><span>Голос бренда<span>{brand.name} · {brand.tone}</span></span><span className="editor-brand-rules" title={brand.rules}>{brand.rules}</span></div>
      ...
      <div className="editor-planning"><label className="editor-field-label" htmlFor="post-status">Этап публикации</label><Select disabled={!canEdit&&!canApprove&&!canSchedule} className="editor-stage-select" id="post-status" value={post.status} onChange={value => patch({status: value as PostStatus})} options={[{value: 'draft', label: 'Черновик',disabled:!canEdit}, {value: 'approved', label: 'Согласовано',disabled:!canApprove}, {value: 'scheduled', label: 'Запланировано',disabled:!canSchedule}]}/>{post.status === 'scheduled' && <><div className="editor-date-fields"><div><span><CalendarDays size={14}/> Дата</span><DatePicker disabled={!canSchedule} aria-label="Дата публикации" min={today} value={post.date} onChange={value => patch({date: value})}/></div><div><span><Clock3 size={14}/> Время</span><TimePicker disabled={!canSchedule} aria-label="Время публикации" value={post.time} onChange={value => patch({time: value})}/></div></div><span className="editor-timezone">Стамбул · UTC+3</span></>}</div>
    ...
    <div className="editor-footer"><div>{existing && canEdit && (deleteConfirm ? <div className="editor-delete-confirm"><span>Удалить публикацию?</span><button onClick={() => {deletePost(post.id);closeEditor();notify('Публикация удалена');}}>Удалить</button><button onClick={() => setDeleteConfirm(false)}>Отмена</button></div> : <button className="editor-delete" onClick={() => setDeleteConfirm(true)}><Trash2 size={16}/><span>Удалить</span></button>)}</div><div><button className="btn btn-ghost" onClick={closeEditor}>Отмена</button><button className="btn editor-publish-button" onClick={()=>setPublishOpen(true)} disabled={!canSchedule||!post.title.trim()||(!post.body.trim()&&!post.media)}><Send size={15}/>Опубликовать</button><button disabled={!canSave||post.status!==source?.status&&!maySetStatus(post.status)} className="btn btn-primary" onClick={save}>{post.status === 'scheduled' ? <CalendarDays size={16}/> : <Check size={16}/>} {post.status === 'draft' ? 'Сохранить черновик' : post.status === 'approved' ? 'Согласовать' : 'Запланировать'}</button></div></div>
    {publishOpen&&<PublishDialog post={post} onClose={()=>setPublishOpen(false)}/>}
  </div>;
}
