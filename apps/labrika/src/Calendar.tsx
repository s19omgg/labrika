...
export default function Calendar() {
  ...
  const {can}=useTeamAccess();const canSchedule=can('schedulePosts'),canCreate=can('createPosts');
  ...
  function movePost(id: string, date: string) {
    ...
    if (!canSchedule || !post || post.status === 'published' || post.date === date) return;
  ...
  function startHold(event: ReactPointerEvent<HTMLButtonElement>, post: Post) {
    if (!canSchedule || post.status === 'published' || event.button !== 0) return;
  ...
  function startNative(event: DragEvent<HTMLButtonElement>, post: Post) {
    if (!canSchedule || post.status === 'published') { event.preventDefault(); return; }
  ...
  async function generatePlan() {
    if(planBusy)return;
    if(!canCreate||!canSchedule)return;
    if (!startDate || !/^\d{4}-\d{2}-\d{2}$/.test(startDate)) { notify('Выберите дату начала плана'); return; }
    setPlanBusy(true);
    try{const result=await requestAI<{posts:{title:string;body:string;topic?:string;format?:string}[]}>({task:'plan',format:'json',context:{brain:getBrain(),previous:posts.slice(0,40).map(p=>({title:p.title,date:p.date,views:p.views}))},prompt:`Создай ${duration} разных готовых публикаций для компании по одной на день с ${startDate}. Площадки: ${planPlatform==='all'?platforms.join(', '):planPlatform}. Соблюдай стиль компании и чередуй цели. Верни JSON {"posts":[{"title":"заголовок","body":"полный текст","topic":"тема","format":"Пост"}]}. Не выдумывай факты, цены, отзывы.`});
    if(!Array.isArray(result.data?.posts)||result.data.posts.length!==duration||result.data.posts.some(p=>!p.title?.trim()||!p.body?.trim()))throw Error('Не удалось подготовить весь план. Повторите запрос.');
    const created=result.data.posts.map((material,i)=>{const day=new Date(`${startDate}T12:00:00`);day.setDate(day.getDate()+i);return makePost(material.title,{id:`plan-${crypto.randomUUID()}`,planned:true,date:toISO(day),time:'12:00',platforms:[planPlatform==='all'?platforms[i%platforms.length]:planPlatform],topic:material.topic||'Компания',format:material.format||'Пост',body:material.body,cover:['lavender','mint','blue','peach'][i%4]});});
    created.forEach(addPost);
    setMonth(new Date(`${startDate}T12:00:00`));
    setPlatform('all');
    setPlanOpen(false);
    notify(`Создано ${duration} черновиков`);
    }catch(error){notify(error instanceof Error?error.message:'Не удалось создать план');}finally{setPlanBusy(false);}
  }

  return <div className={`planning-page calendar-v3${week ? ' calendar-week-view' : ''}`}>
    <div className="page-heading">
      <h1 className="page-title">Контент-план</h1>
      <button className="btn btn-primary" disabled={!canCreate||!canSchedule} onClick={() => setPlanOpen(true)}><Sparkles size={16} /> Создать план</button>
    </div>
        ...
        <div className="calendar-grid">
          {cells.map(day => {
            ...
            return <div className={`calendar-cell${isOutside && !week ? ' outside' : ''}${iso === TODAY ? ' today' : ''}${dragging && dropDate === iso ? ' calendar-drop-target' : ''}`} data-calendar-date={iso} key={iso} onDragOver={event => { if (nativeDrag.current) { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; setDropDate(iso); } }} onDrop={event => { event.preventDefault(); const id = nativeDrag.current || event.dataTransfer.getData('application/x-ygroup-post'); if (id) { movePost(id, iso); suppressClick.current = Date.now() + 600; } clearDrag(); }}>
              ...
              <button disabled={!canCreate||!canSchedule} className="calendar-day-add" aria-label={`Создать пост на ${dateLabel(iso)}`} onClick={() => openEditor(makePost('', { date: iso, planned: true }))}><Plus size={13} /></button>
    ...
    {selectedDay && <Modal title={dateLabel(selectedDay)} onClose={() => setSelectedDay(null)}>
      <div className="calendar-day-dialog">
        ...
        <button disabled={!canCreate||!canSchedule} className="btn btn-primary calendar-dialog-add" onClick={() => { const date = selectedDay; setSelectedDay(null); openEditor(makePost('', { date, planned: true })); }}><Plus size={16} /> Добавить публикацию</button>
      </div>
    </Modal>}
    ...
    {planOpen && <Modal title="Новый контент-план" onClose={() => setPlanOpen(false)}>
      <div className="plan-form">
        ...
        <button className="btn btn-primary plan-submit" disabled={planBusy} onClick={generatePlan}><Sparkles size={16} /> {planBusy?'Готовим план…':`Создать ${duration} черновиков`}</button>
      </div>
    </Modal>}
  </div>;
}
...
function CalendarEvent({ post, onClick, onPointerDown, onDragStart, onDragEnd, onMove, dragging }: { post: Post; onClick: () => void; onPointerDown: (event: ReactPointerEvent<HTMLButtonElement>) => void; onDragStart: (event: DragEvent<HTMLButtonElement>) => void; onDragEnd: () => void; onMove: (offset: number) => void; dragging: boolean }) {
  const {can}=useTeamAccess();const movable = can('schedulePosts')&&post.status !== 'published';
